...
sealed class Screen(val route:String) {
    ...
    data object AgencyProfileScreen: Screen(route = NavPath.USERPROFILESCREEN.name)
    data object ChooseCityInEdit: Screen(route = NavPath.CHOOSECITYINEDIT.name)

    data object AddNewListingScreen: Screen(route = NavPath.ADDNEWLISTINGSCREEN.name)
}